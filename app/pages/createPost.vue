        ...
        <div class="blog-info">
          ...
          <div class="flex justify-start items-center">
           ...
           <div class=" mr-36 relative -mb-4">
            ...
            <div class="file-selector" v-show="postStore.blogPhotoName">
              <div>파일 명:</div>
              <div class="text-align">{{ postStore.blogPhotoName }}</div>
            </div>
          </div>
          </div>          
        </div>
...
import { addDoc, collection, doc, setDoc } from 'firebase/firestore';
  ...
  const blogTitle = computed({
    ...
    set(blogTitle) {
      postStore.updateBlogTitle(blogTitle)
    }    
  })
  ...
  const blogHTML = computed({
    ...
    set(value) {
      postStore.newBlogPost(value)
      console.log(value)
    }
  })  
  const fileChange = (event) => {         
    file.value = event.target.files[0]            
    postStore.fileNameChange(file.value.name) 
